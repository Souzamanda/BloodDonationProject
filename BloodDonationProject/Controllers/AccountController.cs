...
namespace BloodDonationProject.Controllers
{
    public class AccountController : ControllerBase
    {
        ...
        {
            _logger.LogInformation($"Registration Attempt for {accountDTO.Email}");
            ...
            try
            {
                var account = _mapper.Map<ApiUser>(accountDTO);
        ...
        {
            _logger.LogInformation($"Login Attempt for {accountDTO.Email} ");
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            ...
            try
            {
                if (!await _authManager.ValidateUser(accountDTO))
                {
                    return Unauthorized();
                }
